...
export default function ProductDetail() {
  ...
  const thumbEmojis = [product.emoji, ...THUMBS];
    ...
    <div style={{ paddingTop: '64px' }}>
      <div className={styles.layout}>
        ...
        <div className={styles.gallery}>
          <div className={styles.mainImg}>{thumbEmojis[activeThumb]}</div>
          ...
          {PRODUCTS.filter(p => p.id !== product.id && p.cat === product.cat).slice(0, 4).map(p => (
            <div key={p.id} className={styles.relatedCard} onClick={() => { navigate(`/product/${p.id}`); window.scrollTo(0,0); }}>
              ...
              <div style={{ padding: '1rem' }}>
                ...
                <div style={{ color: 'var(--gold)' }}>₹{p.price.toLocaleString('en-IN')}</div>
              </div>
            </div>
          ))}
        </div>
